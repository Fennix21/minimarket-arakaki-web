...
| `panel.html` | CRM del dueño (pass = ARAKAKI_ADMIN_PASS): 🏠 Inicio (dashboard: tarjetas-resumen + últimos pedidos/chats + guía), 💬 Chats, 🛒 Pedidos, ❓ Consultas (preguntas que el chat web no pudo responder; ✓ = revisada y se borra; badge con el total), 💰 Precios (overrides en vivo), 👥 Club, 📊 Analíticas, ⚙️ Bot (prompt editable + avisos + interruptor del chat web + respuestas rápidas + 💬 Chat de la web: saludo/botones/burbuja/subtítulo del widget y cerebro PROPIO independiente de WhatsApp). Menú lateral en escritorio / barra inferior en móvil. Carga /data/catalog.js para la tabla de precios |
...
| `api/crm.js` | Backend del panel: list/get/send/status/rename/note/tags/pause/clearchat/delete/pedidos/pedidoestado/consultas/consultadel/clientes/clientedel/stats/getprompt/setprompt/resetprompt/setnotify/getwebchat/setwebchat/gettemplates/settemplates/getprecios/setprecio |
