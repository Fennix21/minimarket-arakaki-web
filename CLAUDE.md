...
| `panel.html` | CRM del dueño (pass = ARAKAKI_ADMIN_PASS): 🏠 Inicio (dashboard: tarjetas-resumen + últimos pedidos/chats + guía), 💬 Chats, 🛒 Pedidos, ❓ Consultas (preguntas que el chat web no pudo responder; ✓ = revisada y se borra; badge con el total), 💰 Precios (overrides en vivo), 👥 Club, 📊 Analíticas, ⚙️ Bot (3 sub-pestañas: 🤖 WhatsApp = cerebro del bot + respuestas rápidas · 💬 Chat web = interruptor on/off + saludo/botones/burbuja/subtítulo del widget + cerebro PROPIO independiente de WhatsApp · 🔔 Avisos = números autorizados + notify). Menú lateral en escritorio / barra inferior en móvil. Carga /data/catalog.js para la tabla de precios |
