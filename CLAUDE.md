...
| `api/chat.js` | Chat vendedor de la web (widget de site.js). GET → `{on}` (hay ANTHROPIC_API_KEY y config:webchat≠'0'); POST {sid,mensajes} → Claude con el MISMO cerebro del bot (config:prompt + sufijo web) y tools `buscar_productos` (catálogo + precios en vivo) y `registrar_pedido` (LPUSH pedidos con pagina:'chat-web' + aviso a los dueños; devuelve productos_registrados para que el bot verifique; un re-registro de la MISMA sesión REEMPLAZA su pedido 'nuevo' anterior — corrección, no duplica — con aviso 🔁) → CIERRA la venta en la página. Rate limit por sesión (40/h) e IP (120/h) en `chatrl:*`. El historial vive en el navegador, NO en Redis |
