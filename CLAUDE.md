...
| `api/push.js` | Web Push: GET ?key (clave pública VAPID, caché 5min) · POST subscribe/unsubscribe (rol `clientes` público, `duenos` con pass) · send/test/count (admin, pass). `send` acepta `image` (banner grande; se ve en Chrome/Edge/Android, iPhone/Firefox la ignoran). El compositor del panel tiene buscador de fotos del catálogo + vista previa. Tope 5000 suscriptores por rol |
